...
Shader "TheLazyCowboy1/ParallaxDecal" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
sampler2D _DynamicLevelElements;
sampler2D _OrigLevelTex;
#include "_RippleClip.cginc"
#include "_DynamicLevelClip.cginc"
...
dynamicLevelClip(i.scrPos, textCoord);
...
if (texcol.x == 1.0 && texcol.y == 1.0 && texcol.z == 1.0){
   ...
   // modded:
   // The clouds light mask can be stretched. The mask is applied to the full
   // level texture. Merged level textures contain multiple screens.
  ...
  terrainCol = lerp(terrainCol, tex2D(_PalTex, float2(1.5/32.0, 7.5/8.0)), clamp(red*_fogAmount/30.0, 0, 1));
   ...
   half h = textCoord.x;
//TrueParallax MODDED change: don't include this line. It is stupid and has no good reason to exist. Thank you.
   ...
   half4 result = half4(lerp(lightCol, Multiply(terrainCol.xyz, myCol.xyz), lerp(0.2, 0.3, shadow)), myCol.w*lerp(1, 0.25+0.25*i.clr.w, shadow)*i.clr.w);
